...
fn rules_234(input: &str) -> Option<String> {
    ...
    if first_vowel_index.is_none() {
        ...
        return Some(split_join(first_y, input));
    }
    ...
    if let Some(first_q) = start.find("q") {
        if end.starts_with("u") {
            return Some(split_join(first_q + 2, input));
        }
    }
...
pub fn translate(input: &str) -> String {
    ...
    for word in words {
        if let Some(_word) = rule_1(word) {
            out_vec.push(_word);
            continue;
        }
        
        if let Some(word) = rules_234(word) {
            out_vec.push(word);
        }
    }
